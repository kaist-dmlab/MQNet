...
class SCMI(Strategy):
    ...
    def select(self, budget):
        ...
        if (embedding_type == "gradients"):
            ...
            query_embedding = self.get_grad_embedding(self.query_dataset, False, gradType, self.args['class_dict'])
            private_embedding = self.get_grad_embedding(self.private_dataset, False, gradType, self.args['ood_name'])
        ...
        data_sijs = helper.create_kernel(X=unlabeled_data_embedding.cpu().numpy(), metric=metric,
                                                   method="sklearn")
        # Compute query-query kernel
        if (self.args['scmi_function'] == 'logdetcmi'):
            query_query_sijs = helper.create_kernel(X=query_embedding.cpu().numpy(), metric=metric,
                                                              method="sklearn")
            private_private_sijs = helper.create_kernel(X=private_embedding.cpu().numpy(), metric=metric,
                                                                  method="sklearn")
            query_private_sijs = helper.create_kernel(X=private_embedding.cpu().numpy(),
                                                                X_rep=query_embedding.cpu().numpy(), metric=metric,
                                                                method="sklearn")
        # Compute image-query kernel
        query_sijs = helper.create_kernel(X=query_embedding.cpu().numpy(),
                                                    X_rep=unlabeled_data_embedding.cpu().numpy(), metric=metric,
                                                    method="sklearn")
        private_sijs = helper.create_kernel(X=private_embedding.cpu().numpy(),
                                                      X_rep=unlabeled_data_embedding.cpu().numpy(), metric=metric,
                                                      method="sklearn")

        if (self.args['scmi_function'] == 'flcmi'):
            obj = FacilityLocationConditionalMutualInformationFunction(n=unlabeled_data_embedding.shape[0],
                                                                                 num_queries=query_embedding.shape[0],
                                                                                 num_privates=private_embedding.shape[
                                                                                     0],
                                                                                 data_sijs=data_sijs,
        ...
        if (self.args['scmi_function'] == 'logdetcmi'):
            ...
            obj = LogDeterminantConditionalMutualInformationFunction(n=unlabeled_data_embedding.shape[0],
                                                                               num_queries=query_embedding.shape[0],
