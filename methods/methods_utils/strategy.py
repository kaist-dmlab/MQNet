...
class Strategy:
    ...
    def get_grad_embedding(self, dataset, predict_labels, grad_embedding_type="bias_linear", class_dict=None):
        ...
        if predict_labels:
            for batch_idx, (unlabeled_data_batch, _, _) in enumerate(dataloader):
                start_slice = evaluated_instances
            ...
            for batch_idx, (inputs, targets, _) in enumerate(dataloader):
                ...
                '''
                if class_dict == 'imgnet_indice_dm_v3' or class_dict == 'places365_dm_v2':
                    for i in range(targets.shape[0]):
                        targets[i] = 50
                elif class_dict is not None:
                    for i in range(targets.shape[0]):
                        targets[i] = class_dict[targets[i].item()]
                '''
